...
/n/holylabs/kozinsky_lab/Users/lsteinberger/systems/cu_bicrystal/
  data/raw/bicrystal_cu_crystal_liquid.extxyz   1086 atoms
  analysis/graincluster/run_graincluster.py
```

FCC Cu crystal (grain tag 0, 640 atoms) + Poisson disk amorphous grain
(grain tag 1, 446 atoms). Generated by `data/raw/generate_crystal_liquid.py`.
...
Expected:
- Rank 0 cluster: 640 atoms, 100% grain 0 (crystal), entropy ≈ 1.36 nats
- Amorphous grain: several clusters (Poisson disk is sparse → low connectivity
  in amorphous grain → fragmentation expected; not a bug)

Use `grain_tag` init because the bulk FCC bonds (d=2.556 Å) cannot overcome
the singleton merge barrier for M≈256 bins with σ=1.0.
